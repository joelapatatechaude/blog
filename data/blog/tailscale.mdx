...
So really, all that is needed is to precreate this /.config directory, and run the chown / chmod commands explained in the articlue.
Hopefully Tailscale will eventually implement this, but for the time being, a small hack is required. I went with the
...
Once the operator is installed, there are several way you can use it. In my case, I want three things:

1. Have acces to the console
...
This github issue https://github.com/tailscale/tailscale/issues/10814 indicates that the current workaround is to run the tailscale contaienr with privileged mode.
I am pretty sure there should be some more restricitve mode that the blank privileged mode, but for the time being I'll go with that. To achieve this,
we need to create a tailscale proxyClass that let us overide some of the pods settings. Create a ocp-proxyclass.yaml file with
...
- ssl termination on those OpenShift route doens't happen at the service level, it happens at the ha-proxy level. So my let's encrypt certificate is not in use, and the certificate is not trusted.
- I would need to maintain some dns entry for every service I expose (console, oauth, and more), and annoyingly, each service would get a different machine IP on tailscale. This add some burden to maintain some dns entry somewhere.
...
Unless Tailscale had fixed this [issue](https://github.com/tailscale/tailscale/issues/10814), make sure to follow the previous section to add and use a proxyclass, and configure the proxies service account securityContextConstraints.
...
am using regular `oc login` to authenticate / authorize. Tailscale instructions are here: https://tailscale.com/kb/1437/kubernetes-operator-api-server-proxy#configuring-api-server-proxy-in-noauth-mode
...
Once this is done, the tailscole operator will restart, it contains the api proxy activated by the previous command.

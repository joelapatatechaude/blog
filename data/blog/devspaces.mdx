...
requirements in any job advert is insane. This has led to the rise of the full stack developer, jack of all trades master of none.
...
and resolve conflicts? No-one does.
...
solution for new projects, or solutions for legacy projects in need of modernisation.
...
- Define your library of useful commands: Those helpers command that developer constantly rely for their work (compile command lines, debugs, test, cleanup and load db etc...)
...
and maintaining that, fixing dependencies, fighting version conflicts, adding that Database or that message broker to it. That's enough.

This issue has been captured a while back by this xkcd [comic](https://xkcd.com/1987/)
...
What's crazy? Nowadays, each git project has typically a lengthy, out-of-date README to help setup the environment. This README is the thing that
a developer environment should be project specific, it makes perfect sense to containerise it, for each project, right?

## Breaking silos

Forgive me for using a buzz word, but that's just true. One reasons teams and projects are siloed
is the difficulty to get started on other teams project. Sometime just to do some small changes or
bug fixes. Sometime to learn for that other project, test a variation. Being able to instantiate
a configured and ready to use IDE for any project, at the click of a button (last buzzword today
I promiss) is an invitation to explore and collaborate on anyone's project.
...
- native apps, such as the postman app, can't be containerized to run on OpenShift. Only CLI tools, or web based applications can run as side care containers.
  ...
  has had a [web based offering](https://identity.getpostman.com/signup) since a while. Alternatives can be used as VSCode extensions too.
...
smile and think "how did we do that??"... How did we do music before audio streaming platforms?
Ah yes, we had those vinyl records, clunky cassette tapes, then later CDs to carry around with us,
life is so convenient these days.
How did we do before the likes of office 365, google doc etc...
...
Here adoption is the key, the solution is ready but will you make the shift to DevSpaces on OpenShift?

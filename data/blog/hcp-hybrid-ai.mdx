...
<figure>
  {' '}
  <img
    src="/static/images/hcp-hybrid-ai/gpu-cloud-burst.png"
    alt="Hybrid architecture: on-prem control plane connected to elastic cloud GPU capacity"
    width="800"
  />
</figure>
...
- Worker nodes in AWS, including GPU workers consumed by [OpenShift AI](https://www.redhat.com/en/technologies/cloud-computing/openshift/openshift-ai)
...
<figure>
  {' '}
  <img
    src="/static/images/hcp-hybrid-ai/gpu-dust.png"
    alt="Expensive GPU servers sitting idle in a data center"
    width="800"
  />
</figure>
...
<figure>
  {' '}
  <img
    src="/static/images/hcp-hybrid-ai/arch.png"
    alt="HCP hybrid AI architecture: on-prem control plane with elastic AWS GPU workers"
    width="800"
  />
</figure>
...
- When I no longer need the GPU capacity, I scale the node pool down to zero — or better, let OpenShift [do it automatically](https://developers.redhat.com/articles/2026/02/26/how-hosted-control-planes-are-getting-smarter-about-resource-management).
...
<figure>
  {' '}
  <img
    src="/static/images/hcp-hybrid-ai/gpu-evolution.png"
    alt="GPU accelerator generations evolving rapidly"
    width="800"
  />
</figure>

The current enterprise AI conversation is dominated by high-end GPUs such as [H100](https://www.nvidia.com/en-us/data-center/h100/) and [H200](https://www.nvidia.com/en-us/data-center/h200/). Those are powerful accelerators, and they will remain useful for a long time. But the market is not standing still.
...
<figure>
  {' '}
  <img
    src="/static/images/hcp-hybrid-ai/gpu-cloud-choice.png"
    alt="The choice between permanent on-prem GPU infrastructure and elastic cloud capacity"
    width="800"
  />
</figure>
...
<figure>
  {' '}
  <img
    src="/static/images/hcp-hybrid-ai/puppet.png"
    alt="Vendor lock-in versus platform portability"
    width="800"
  />
</figure>
...
<figure>
  {' '}
  <img
    src="/static/images/hcp-hybrid-ai/homelab.png"
    alt="Home lab setup for the hosted control plane PoC"
    width="800"
  />
</figure>
...
I start with two standard EC2 replicas — enough to deploy operators and workloads. Once the cluster is up and running, it becomes easy to add additional node pools via the [Advanced Cluster Management](https://www.redhat.com/en/technologies/management/advanced-cluster-management) console (ACM), including GPU-capable node pools.
...
In an enterprise or government AWS environment, approved private connectivity patterns such as [AWS Direct Connect](https://aws.amazon.com/directconnect/) or site-to-site VPN would typically be in place.

For a quick PoC from a home lab, something like [Tailscale](https://tailscale.com/) with subnet routers can bridge on-prem with AWS instances. Be mindful that this is typically not an approved network path at work — but it is a practical way to validate the pattern before investing in production connectivity.
...
- GPU-enabled workloads such as [vLLM](https://docs.vllm.ai/) inference services

The workflow looks like this:

1. Bring up the hosted cluster.
2. Scale up an AWS GPU worker node pool (e.g. `g6.xlarge` with NVIDIA L4 as a cost-effective GPU node to get started with).

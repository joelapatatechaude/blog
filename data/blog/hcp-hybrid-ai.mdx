...
- A subnet router on the local side advertising the on-prem network (e.g. `192.168.6.0/24`).
- A subnet router in the AWS VPC advertising the VPC CIDR (e.g. `10.1.0.0/16`).
- IP forwarding enabled on both subnet routers.
- Static routes on the OpenShift nodes so they know how to reach the AWS VPC, typically applied via a MachineConfig with a NetworkManager dispatcher script.

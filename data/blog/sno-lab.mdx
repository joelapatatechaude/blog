...
- name it something like 98_var_partition.yaml. The first two number (98) will make sure this doesn't run before some machined configuration
  that should run earlier.
...
For this installation, the internal image registry is not automatically created. Pretty sure it's due to the lack of storage class, up to now.
To enable it, just follow the documentation:
https://docs.redhat.com/en/documentation/openshift_container_platform/4.16/html-single/registry/index#registry-configuring-storage-baremetal_configuring-registry-storage-baremetal
...
I don't think that's possible, I didn't find any hint on how to do that. At this stage, your SNO needs to be attached
to your router via the ethernet cable. It's a good time to enable Wake on Lan (WOL) on your laptop bios, to be able to start
